import React from "react";

export const Footer = () => {

    return (
        <div className={'flex-[0_0_auto] text-[12px] text-white font-[Mont]'}>
            <div className={'wrapper max-w-none min-w-fit px-[1em]'}>
                <div className={'foot-wrap justify-center border-t border-black/[.5] flex pt-[1.8em] pb-[2.3em]'}>
                    <div className={'foot-outro float-none text-center flex flex-col justify-between'}>
                        <div  className={'foot-logo inline-block float-none py-[.2em] mt-[.7em] mb-[1.35em] text-[1.35em]'}>
                            Company
                        </div>
                        <div>

                        </div>
                        <div className={'copyright text-center mt-[1.2em] clear-both font-[MontL] opacity-50 text-[.72em]'}>
                            © 2018–2024{' '}
                            <span>FixedFloat</span>
                            . All rights reserved.
                        </div>
                    </div>
                </div>
            </div>
        </div>
    )
}
